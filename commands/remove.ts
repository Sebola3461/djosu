...
export default new SlashCommand()
	...
	.setExecutable(async (command) => {
		...
		if (removeStatus == SongRemoveStatus.Destroyed) {
			command.deleteReply();
			return queue.finalizeQueue(true);
		}
		...
		if (removeStatus == SongRemoveStatus.None) {
			const embed = new EmbedBuilder()
				...
				.setTitle(
					`${targetSong.beatmapInfo.artist} - ${targetSong.beatmapInfo.title}`
				)
				.setURL(
					`https://osu.ppy.sh/beatmapsets/${targetSong.beatmapInfo.id}`
				)
				.setThumbnail(
					`https://b.ppy.sh/thumb/${targetSong.beatmapInfo.id}l.jpg`
				)
